...
RtG-CLI identifies `image` as addon.

`--width 128` corresponds to the RtG Image interface and must be processed by said addon.
...
Arguments must be delivered to the addon in the order in which they were provided by the user.
